    ...
    function createSchemaInit (axisArray,reset){ // input {text: 'axis',range:[],filter:[],angle:0}
        ...
        axisArray.forEach((axiselement,index)=>{
            ...
            radarcomp.axis[axis] = {
                ...
                range: axiselement.range!==undefined?axiselement.range:[0,1],
            ...
            dataTable = $(table.node()).DataTable({
                ...
                "columnDefs": [
                    ...
                    {   targets: 1,
                        ...
                        "render": function ( d, type, row, meta ) {
                            if (type=='display') {
                                return '<input type="number" value=' + d.range[0] + '></input>';
                            }
                            else
                                return d.range[0];
                        }
                    ...
                    {   targets: 2,
                        ...
                        "render": function ( d, type, row, meta ) {
                            if (type=='display') {
                                return '<svg class="s_chart" width="80" height="25"></svg>';
                            }
                    ...
                    {   targets: 3,
                        ...
                        "render": function ( d, type, row, meta ) {
                            if (type=='display') {
                                return '<input type="number" value=' + d.range[1] + '></input>';
                            }
                            else
                                return d.range[1];
                        }
...
// <label class="col left-align" style="margin-top:6px"> <input id="dataRange_control" type="checkbox" class="filled-in"> <span>Remove isngle value</span> </label>`);
    ...
    let violiin_chart = d3.viiolinChart().graphicopt({width:80,height:25,opt:{dataformated:true},margin: {top: 0, right: 0, bottom: 0, left: 0},middleAxis:{'stroke-width':0.5},ticks:{'stroke-width':0.5},tick:{visibile:false}});;
    function eventTable(){
