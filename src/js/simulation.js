        ...
        if (!this.isRealTime) {
            ...
            let updatePromise = (_.isString(url) ? d3.json(url) : url).then((data) => {
                ...
                d3.keys(data.jobs_info).forEach(jID => {
                    ...
                    }else if (data.jobs_info[jID].start_time < 9999999999) {
                        data.jobs_info[jID].start_time = data.jobs_info[jID].start_time * 1000
                        data.jobs_info[jID].submit_time = data.jobs_info[jID].submit_time * 1000
                        if (data.jobs_info[jID].finish_time && data.jobs_info[jID].finish_time < 9999999999)
